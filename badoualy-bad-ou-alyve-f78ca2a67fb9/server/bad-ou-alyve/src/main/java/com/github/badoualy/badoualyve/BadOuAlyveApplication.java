...
public class BadOuAlyveApplication extends WebMvcConfigurerAdapter {
    ...
    public HomeMessage home() {
        ...
        homeMessage.addOperation("/users/{name}/{lines}/{scores}", "Get self as {name} the partie");
    ...
    @RequestMapping("/users/{name}/{lines}/{scores}")
    @ResponseBody
    public Partie user(@PathVariable final String name, @PathVariable final String lines,@PathVariable final String scores) throws IOException {
        Logger.getLogger(LOGGER_NAME).info("Getuser(): " + name+"   line   "+  lines +"  scores  "+scores);
        long ln =Long.parseLong(lines);
        long scr = Long.parseLong(scores);
        gameHandler.updateuser(name,ln,scr);
